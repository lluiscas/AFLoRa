...
public:
    ...
    // (LC)
    int lastSeqProcessed;
    int lastSentSeqNumber;
    //
...
class INET_API NetworkServerApp : public cSimpleModule, cListener
{
  protected:
    ...
    int totalReceivedFrames1GW;  // (LC)
    int totalRcvdConfFrames1GW;
    int totalRcvdUnconfFrames1GW;
    cOutVector rcvdPkConfd;      // (LC) vector: confirmed packets received by NS App
    cOutVector rcvdPkUnConfd;    // (LC) vector: unconfirmed packets received by NS App
    cOutVector rcvdFrConfd;      // (LC) vector: confirmed frames received by NS
    cOutVector rcvdFrUnConfd;    // (LC) vector: unconfirmed frames received by NS
    std::string adrMethod;
    double adrDeviceMargin;
    // (LC)
    int noAckResponseCounter;
    double iniTP;    // 2 .. 14   (dBm)
    double TPnow;    // 2 .. 14   (dBm)
  ...
  protected:
    ...
    bool isPacketNew(LoRaMacFrame* pkt);
    ...
    // (LC)
    void ackmentPacket(LoRaMacFrame* pkt, L3Address pickedGateway, double SNIRinGW, double RSSIinGW);
    void unConfPacket(LoRaMacFrame* pkt, L3Address pickedGateway, double SNIRinGW, double RSSIinGW);
